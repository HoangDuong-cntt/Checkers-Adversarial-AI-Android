...
public class GameEngine {
    ...
    public static Board makeMove(Board board, Move move) {
        ...
        // XỬ LÝ NHẢY NHIỀU BƯỚC (MULTI-JUMP) TỰ ĐỘNG
        if (move.isJump) {
            List<Move> nextJumps = getLegalJumpsForPiece(board, move.toRow, move.toCol);
            if (!nextJumps.isEmpty()) {
                // Nếu chỉ có đúng 1 lựa chọn nhảy tiếp, tự động thực hiện (đáp ứng yêu cầu "tự nhảy")
                if (nextJumps.size() == 1) {
                    return makeMove(board, nextJumps.get(0));
                }
                // Nếu có nhiều hơn 1 lựa chọn, giữ nguyên lượt để người chơi/AI chọn hướng nhảy tiếp theo
                return board;
            }
        }
